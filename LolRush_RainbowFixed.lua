...
-- 彩虹背景函数
local function applyRainbowBackground(uiElement)
    local hue = 0
...
-- 缩小 UI 尺寸
lolrushtheme.Size = UDim2.new(0, 350, 0, 300)
lolrushtheme.Position = UDim2.new(0.5, -175, 0.5, -150)

-- 改滑块颜色为绿色
for _, obj in pairs(lolrushtheme:GetDescendants()) do
    if obj:IsA("TextButton") and obj.Name == "SliderInner" then
        obj.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
    end
end
...
local function onToggleChanged(isEnabled)
    if isEnabled then
        applyRainbowBackground(lolrushtheme)
    else
        TweenService:Create(lolrushtheme, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}):Play()
    end
end

-- 示例绑定 Toggle
-- Toggle("彩虹模式", nil, function(value)
--     onToggleChanged(value)
-- end)


-- [修复：集成 Toggle 控制彩虹 UI] --
local rainbowToggle = Instance.new("BoolValue")
rainbowToggle.Name = "RainbowEnabled"
rainbowToggle.Value = false
rainbowToggle.Parent = lolrushtheme

-- 简易绑定函数（你可以换成你 GUI 库的 Toggle 实现）
local function simulateToggleButton()
    rainbowToggle.Value = not rainbowToggle.Value
...
testButton.Text = "Toggle Rainbow"
testButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
testButton.TextColor3 = Color3.new(1, 1, 1)
testButton.Parent = lolrushtheme
testButton.MouseButton1Click:Connect(simulateToggleButton)
